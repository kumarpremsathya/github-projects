...
#         query = f"SELECT * from pop_with_pfrda"
...
#             if not row["New Reg No."] in database_df["New_Reg_No"]:
#                 missing_rows_in_db.append(row)

#         for index, row in database_df.iterrows():
#             if not row["New_Reg_No"] in excel_df["New Reg No."]:
#                 missing_rows_in_excel.append(row)

#         print(len(missing_rows_in_db),"missing rows in db")
#         print(len(missing_rows_in_excel),"missing row in excel")
...
#             sys.exit()
#         elif len(missing_rows_in_excel) == 0 and len(missing_rows_in_db) == 0:
#             pop_config.log_list[1] = "Success"
#             pop_config.log_list[4] = get_data_count_database.get_data_count_database(pop_config.cursor)
...
#         pop_config.no_data_avaliable = len(missing_rows_in_excel)
#         pop_config.no_data_scraped = len(missing_rows_in_excel)

#         current_date = datetime.now().strftime("%Y-%m-%d")
#         increment_file_name = f"incremental_excel_sheet_{current_date}.xlsx"
#         increment_data_excel_path = fr"C:\Users\mohan.7482\Desktop\PFRDA\List of entities registered as POP with PFRDA\data\increment_data\{increment_file_name}"
#         excel_df.to_excel(increment_data_excel_path,index=False)
...
def check_increment_data(excel_path):
    try:
        ...
        current_date = datetime.now().strftime("%Y-%m-%d")
        increment_file_name = f"incremental_excel_sheet_{current_date}.xlsx"
        increment_data_excel_path = fr"C:\Users\mohan.7482\Desktop\PFRDA\Aggregaters PFRDA\data\increment_data\{increment_file_name}"
